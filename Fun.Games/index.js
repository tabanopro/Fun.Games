(async()=>{
    ...
    s4d.client.on('ready', async () => {
        ...
        if (randomchange == 1) {
          s4d.client.user.setPresence({status: "online",activities:[{name:'Fun Games at Fun.Games',type:"PLAYING"}]});
        } else if (randomchange == 2) {
          s4d.client.user.setPresence({status: "dnd",activities:[{name:'WITH FUN.GAMES PLAYER',type:"COMPETING"}]});
        } else if (randomchange == 3) {
          s4d.client.user.setPresence({status: "idle",activities:[{name:'Select a game at Fun.Games',type:"COMPETING"}]});
        } else if (randomchange == 4) {
          s4d.client.user.setPresence({status: "online",activities:[{name:'To Justice on Soundcloud',type:"LISTENING"}]});
        } else if (randomchange == 5) {
          s4d.client.user.setPresence({status: "online",activities:[{name:' With Time',type:"COMPETING"}]});
        }
    ...
    s4d.client.on('interactionCreate', async (interaction) => {
        ...
        (interaction.channel).send({ content: String('Here are the available options:'),
